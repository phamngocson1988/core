...
class FetchCompletedShopForm extends FetchShopForm
{
    ...
    {
        $this->status = OrderSupplier::STATUS_COMPLETED;
    }
    protected function createCommand()
    {
        ...
        $command->select([
            ...
            "TIMESTAMPDIFF(MINUTE , $supplierTable.completed_at, '$now') as confirmed_time",
        ]);
        ...
        if ($this->start_date) {
            $command->andWhere(['>=', "$supplierTable.completed_at", $this->start_date]);
        }
        if ($this->end_date) {
            $command->andWhere(['<=', "$supplierTable.completed_at", $this->end_date]);
        }
