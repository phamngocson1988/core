      ...
      <div class="portlet-title">
        ...
        <div class="actions">
          <div class="btn-group btn-group-devided">
            <a class="btn purple" href="<?=Url::to(['task/index', 'assignee' => Yii::$app->user->id, 'status' => ['new', 'inprogress']])?>"><?=Yii::t('app', 'my_open_tasks')?></a>
          </div>
          <div class="btn-group btn-group-devided">
            <a class="btn green" href="<?=Url::to(['task/create', 'ref' => $ref])?>"><?=Yii::t('app', 'add_new')?></a>
          </div>
        </div>
      </div>
      <div class="portlet-body">
        <div class="row margin-bottom-10">
          <?php $form = ActiveForm::begin(['method' => 'GET', 'action' => ['task/index']]);?>     
            <?php $creator = $search->getCreator();?>
            <?=$form->field($search, 'created_by', [
              'options' => ['class' => 'form-group col-md-3'],
            ])->widget(kartik\select2\Select2::classname(), [
              'initValueText' => ($search->created_by) ? sprintf("%s - %s", $creator->username, $creator->email) : '',
              'options' => ['class' => 'form-control', 'name' => 'created_by'],
              'pluginOptions' => [
            ...
            <?=$form->field($search, 'assignee', [
              ...
              'initValueText' => ($search->assignee) ? sprintf("%s - %s", $assignee->username, $assignee->email) : '',
              'options' => ['class' => 'form-control', 'name' => 'assignee'],
              'pluginOptions' => [
                'placeholder' => Yii::t('app', 'select_assignee'),
                'allowClear' => true,
                'minimumInputLength' => 3,
              ...
              <tr>
                ...
                <td style="vertical-align: middle; text-align: center;">
                  ...
                  <div class="progress progress-striped active">
                      <div class="progress-bar <?= $model->isDelay() ? 'progress-bar-danger' : 'progress-bar-success';?>" role="progressbar" aria-valuenow="<?=$model->percent?>" aria-valuemin="0" aria-valuemax="100" style="width: <?=$model->percent?>%">
                ...
                <td style="vertical-align: middle;">
                  <a href="<?=Url::to(['task/edit', 'id' => $model->id, 'ref' => $ref])?>" class="btn btn-xs grey-salsa tooltips" data-container="body" data-original-title="<?=Yii::t('app', 'edit_task')?>"><i class="fa fa-pencil"></i></a>
                  <a class="btn btn-xs grey-salsa tooltips" data-container="body" data-original-title="<?=Yii::t('app', 'view')?>" data-toggle="modal" href="#todo-task-modal" data-object="<?=json_encode(['id'=>$model->id, 'title'=>$model->title, 'description'=>$model->description, 'due_date'=>$model->due_date, 'assignee_name'=>$model->getReceiverName(), 'status'=>$model->status])?>"><i class="fa fa-eye"></i></a>
                  <a href="<?=Url::to(['task/update-status', 'id' => $model->id, 'status' => 'inprogress', 'ref' => $ref])?>" class="btn btn-xs grey-salsa change-status tooltips" data-container="body" data-original-title="<?=Yii::t('app', 'change_to_inprogress')?>"><i class="fa fa-play-circle"></i></a>
                  <a href="<?=Url::to(['task/update-status', 'id' => $model->id, 'status' => 'done', 'ref' => $ref])?>" class="btn btn-xs grey-salsa change-status tooltips" data-container="body" data-original-title="<?=Yii::t('app', 'change_to_done')?>"><i class="fa fa-check"></i></a>
                  <a href="<?=Url::to(['task/update-status', 'id' => $model->id, 'status' => 'invalid', 'ref' => $ref])?>" class="btn btn-xs grey-salsa change-status tooltips" data-container="body" data-original-title="<?=Yii::t('app', 'change_to_invalid')?>"><i class="fa fa-ban"></i></a>
                </td>
              </tr>
              <?php endforeach;?>
          </tbody>
