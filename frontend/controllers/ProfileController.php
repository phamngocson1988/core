...
class ProfileController extends Controller
{
    ...
    public function actionIndex()
    {
        ...
        return $this->render('index', [
            'model' => $model,
            'passwordModel' => new \frontend\forms\ChangePasswordForm()
        ]);
    }
    ...
    public function actionRequestSmsCode()
    {
        ...
        $model = new \frontend\forms\VerifyPhoneForm(['phone' => $phone]);
        $model->setScenario(\frontend\forms\VerifyPhoneForm::SCENARIO_SEND);
    ...
    public function actionVerifySmsCode()
    {
        ...
        if (!$request->isAjax) throw new BadRequestHttpException("Error Processing Request", 1);
        if (!$request->isPost) throw new BadRequestHttpException("Error Processing Request", 1);
        $model = new \frontend\forms\VerifyPhoneForm();
        $model->setScenario(\frontend\forms\VerifyPhoneForm::SCENARIO_VERIFY);
        if ($model->load(Yii::$app->request->post()) && $model->validate()) {
            if ($model->verify()) {
                return json_encode(['status' => true]);
            }
    ...
    public function actionRequestEmailCode()
    {
        ...
        $email = $user->email;
        $model = new \frontend\forms\VerifyEmailForm(['email' => $email]);
        $model->setScenario(\frontend\forms\VerifyEmailForm::SCENARIO_SEND);
        if ($model->validate() && $model->send()) {
            return json_encode(['status' => true]);
        } else {
            $errors = $model->getErrorSummary(true);
    ...
    public function actionVerifyEmailCode()
    {
        ...
        if (!$request->isAjax) throw new BadRequestHttpException("Error Processing Request", 1);
        if (!$request->isPost) throw new BadRequestHttpException("Error Processing Request", 1);
        $model = new \frontend\forms\VerifyEmailForm();
        $model->setScenario(\frontend\forms\VerifyEmailForm::SCENARIO_VERIFY);
        if ($model->load(Yii::$app->request->post()) && $model->validate()) {
            if ($model->verify()) {
                return json_encode(['status' => true]);
            }
