...
class OrderPaymentForm extends Model
{
    ...
    public function purchase()
    {
        ...
        try {
            ...
            if ($paygate->getIdentifier() == 'kinggems') {
                /**
                 * Proceed a withdraw from user wallet
                 * Move order from verifying to pending
                 */
                ...
                /**
                 * Create a 'pending' transaction to deposit kingcoin
                 * Create a relationship between the new transaction and order
                 */
                $depositWallet = new \frontend\forms\WalletPaymentForm([
                    'quantity' => $order->sub_total_price,
                    'paygate' => $paygate->getIdentifier(),
                    'order_id' => $order->id,
                ]);
                $depositWallet->setPaygate($paygate);

                if (!$depositWallet->validate()) {
                    throw new BadRequestHttpException("Error Processing Request", 1);
                }
                $trnId = $depositWallet->purchase();
