...
class DUP_PRO_Archive
{
    ...
    public function buildFile($package, $build_progress)
    {
        ...
        try {
            $this->Package = $package;
            if (!isset($this->PackDir) && !empty($this->PackDir) && !is_dir($this->PackDir)) {
    ...
    private function addToDirList($dirPath, $relativePath, $size, $nodes)
    {
        ...
        if (!empty($relativePath) && !DUP_PRO_Secure_Global_Entity::getInstance()->skip_archive_scan) {
            ...
            if ($name === 'wp-admin') {
                ...
                if ($parent_dir != realpath(duplicator_pro_get_home_path())) {
    ...
    public static function getJsTreeStructure($treeObj, $notFoundText = '', $addFullLoaded = true)
    {
        ...
        switch (count($treeList)) {
            ...
            default:
                ...
                $result   = array(
                    //'id'          => 'no_child_founds',
                    'text'     => empty($rootPath) ? '/' : $rootPath, // node text
                    'type'     => 'folder',
                    'children' => array(),
                    'state'    => array(
    ...
    public function getLocalDirPath($dir, $basePath = '')
    {
        ...
        return ltrim(str_replace($this->PackDir, $basePath, $safeDir), '/');
    }

    public function getLocalFilePath($file, $basePath = '')
    {
        $safeFile = DupProSnapLibIOU::safePathUntrailingslashit($file);
        return ltrim(str_replace($this->PackDir, $basePath, $safeFile), '/');
    }
}