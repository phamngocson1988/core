...
if (empty($archive_file)) {
    ...
    $installer_file_path = '';
    if (file_exists($home_path.'/installer.php')) {
        $installer_file_path = $home_path.'/installer.php';
    }
    if (file_exists($home_path.'/dpro-importinstaller.php')) {
        $installer_file_path = $home_path.'/dpro-importinstaller.php';
    }
    if (!empty($installer_file_path)) {
        ...
        if (preg_match('/const[\s\t]+ARCHIVE_FILENAME[\t\s]*?=[\t\s]*?[\'"](.+)[\'"]/', $installer_file_data, $match)) {
            ...
            $temp_archive_file_path = $home_path.'/'.$temp_archive_file;
...
$archive_path        = empty($archive_file) ? '' : $home_path . '/' .$archive_file;
